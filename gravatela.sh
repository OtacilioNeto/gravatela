...
if [ $# = 0 ] && [ -f $TMP/$USER.gravatela.pid ]; then
	...
	if [ -f $OUT/$USER.VideoAudio.mp4 ]; then
		unlink $OUT/$USER.VideoAudio.mp4
	fi
	touch $TMP/$USER.VideoAudio.mp4
	ln -s $TMP/$USER.VideoAudio.mp4 $OUT/$USER.VideoAudio.mp4
	if [ -f $TMP/$USER.gravacamera.pid  ]; then
		...
		if [ $GRAB = "x11grab" ]; then
            ...
            ffmpeg -y -loglevel error -f avfoundation -framerate 30 -pix_fmt nv12  -i "$SCREENDEVICEINDEX:0" -async 88200  -c:v libx264 -crf 0 -preset ultrafast $TMP/$USER.VideoAudio.mp4 &	
		fi
	fi
	...
	if [ -f $OUT/$USER.CameraAudio.mp4 ]; then
		unlink $OUT/$USER.CameraAudio.mp4
	fi
    touch $TMP/$USER.CameraAudio.mp4
    ln -s $TMP/$USER.CameraAudio.mp4 $OUT/$USER.CameraAudio.mp4
	if [ $GRAB = "x11grab" ]; then
		if [ -f $TMP/$USER.gravatela.pid  ]; then	
			...
			ffmpeg -y -loglevel error -video_size 1920x1080 -framerate 30 -vsync 1 -vcodec h264 -f v4l2 -i $1 -c:v copy /home/ota/Desktop/$USER.CameraAudio.mp4 &
		else
			# Não está gravando a tela, então leia do microfone
			ffmpeg -y -loglevel error -video_size 1920x1080 -framerate 30 -vsync 1 -vcodec h264 -f v4l2 -i $1 -f alsa -ar 44100 -ac 2 -async 44100 -i hw:0 -c:v copy /home/ota/Desktop/$USER.CameraAudio.mp4 &
		fi
	else
		...
		if [ $DEVICE = "HDProWebcam" ]; then
			...
			if [ -f $TMP/$USER.gravatela.pid  ]; then
                ...
                ffmpeg -y -loglevel error -video_size 1920x1080 -framerate 30 -vsync 1 -vcodec h264 -f avfoundation -pix_fmt nv12 -i "$SCREENDEVICEINDEX:1" -async 88200  -c:v copy /Users/otacilio/Desktop/$USER.CameraAudio.mp4 &
            fi
		else
